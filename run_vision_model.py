...
MODEL_REGISTRY = {
    ...
    'dinov2-s':  {'params_m': 21.0,  'dim': 384,  'npu': True,  'ms': 0.855, 'auroc': '—',     'quant': 'XINT8'},
    'dinov2-b':  {'params_m': 86.0,  'dim': 768,  'npu': False, 'ms': 0.0,   'auroc': '—',     'quant': 'XINT8'},
    'clip-b32':  {'params_m': 151.0, 'dim': 512,  'npu': False, 'ms': 0.0,   'auroc': '—',     'quant': 'XINT8'},
    'clip-l14':  {'params_m': 428.0, 'dim': 768,  'npu': False, 'ms': 0.0,   'auroc': '—',     'quant': 'XINT8'},
...
def benchmark(model, res=224, n=100):
    ...
    with torch.no_grad():
        for _ in range(n):
            t0=time.perf_counter(); model(dummy); lats.append((time.perf_counter()-t0)*1000)
    return {'median':np.median(lats),'p95':np.percentile(lats,95),'mean':np.mean(lats)}
...
def main():
    ...
    for name in names:
        ...
        print(f"\nLoading {name} ({spec['params_m']:.1f}M)...")
