...
def train(
    ...
    save_path:    str   = r"checkpoints\dinov2_student\student_dualhead_best.pt",
):
    print("\nSprint 6b — Dual-Head CLIP Distillation")
    print("=" * 50)
    ...
    for ep in range(epochs):
        ...
        for batch in iter_frames(hdf5_dir, max_files, batch_size):
            ...
            # ── L_clip only — no L_preserve needed (backbone frozen) ──────────
            L_clip = cosine_loss(semantic_emb, clip_proj.detach())

            if not torch.isfinite(L_clip):
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-path",
                        default=r"checkpoints\dinov2_student\student_dualhead_best.pt")
