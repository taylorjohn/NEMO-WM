# -*- coding: utf-8 -*-
"""
quantize_cardiac_nolayernorm.py - CORTEX-PE v16.15
Re-quantize CardiacStudentEncoder with real calibration data,
excluding LayerNorm from quantization to improve cosine fidelity.
"""
...
# Load real cardiac clips for domain-matched calibration
calib_clips = []
TARGET_SR, CLIP_LEN = 2000, 2000
for wav in sorted(pathlib.Path('./cardiac_data').rglob('*.wav'))[:500]:
    try:
...
# Exclude LayerNorm from quantization
# Conv and Gemm stay XINT8, LayerNorm stays float32
# One CPU fallback for LayerNorm vs completely broken model
config = QConfig.get_default_config('XINT8')
config.global_quant_config.op_types_to_quantize = [
    'Conv', 'Gemm', 'MatMul', 'Add', 'Mul',
    'Relu', 'Sigmoid', 'Tanh',
    'AveragePool', 'GlobalAveragePool',
    'ReduceMean', 'Squeeze', 'Unsqueeze',
...
class CardiacStudentEncoder(nn.Module):
    def __init__(self):
        ...
        self.proj = nn.Sequential(nn.Linear(256, 768), nn.LayerNorm(768))
    def forward(self, x):
        return self.proj(self.encoder(x).squeeze(-1))


ckpt  = torch.load('./checkpoints/cardiac/student_best.pt',
                   map_location='cpu', weights_only=True)
model = CardiacStudentEncoder()
model.load_state_dict(ckpt['model_state_dict'], strict=True)
model.eval()

# Verify on real audio
x_np = calib_clips[0].reshape(1, 1, -1)
with torch.no_grad():
    pt_out = model(torch.from_numpy(x_np)).numpy().flatten()

sess     = ort.InferenceSession('./npu_models/cardiac_student_xint8.onnx',
...
status = 'OK' if cos > 0.95 else 'WARNING'
print(f'Cosine XINT8 vs PT: {cos:.4f}  {status}')
