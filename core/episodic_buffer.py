...
class EpisodicStore:
    ...
    def __init__(self, capacity: int = EPISODIC_CAPACITY):
        ...
        self._faiss_index = faiss.IndexFlatIP(D_BELIEF)
    ...
    def store(self, b_t: torch.Tensor, a_t: torch.Tensor,
        ...
        if len(self.episodes) >= self.capacity:
            ...
            vec = F.normalize(b_t.detach().float().unsqueeze(0), dim=-1).numpy()
    ...
    def retrieve(self, query_b: torch.Tensor, k: int = 3,
        ...
        # Rebuild FAISS index if eviction happened
        if self._index_dirty:
            self._faiss_index = faiss.IndexFlatIP(D_BELIEF)
            vecs = torch.stack([
                F.normalize(ep.b_t.float().unsqueeze(0), dim=-1).squeeze(0)
                for ep in self.episodes
            ]).numpy()
            self._faiss_index.add(vecs)
            self._index_dirty = False

        # FAISS search — get k*3 candidates
        n_candidates = min(k * 3, len(self.episodes))
        q_vec = F.normalize(query_b.float().unsqueeze(0), dim=-1).numpy()
        sims, indices = self._faiss_index.search(q_vec, n_candidates)

        # Re-rank with recency + priority
        n = len(self.episodes)
        scored = []
