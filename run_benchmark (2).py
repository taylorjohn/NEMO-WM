...
# PushT uses terminal-only cost — regime-switching dynamics make
# intermediate cost misleading (Wang et al. Section B.1)
TERMINAL_ONLY_ENVS = {"pusht"}
...
# =============================================================================
# MeZO planner — Memory-Efficient Zero-Order optimisation
#
# Instead of backpropagating through the predictor (which requires holding
# the full computation graph in memory and computing gradients through H=25
# predictor calls), MeZO estimates the gradient using only 2 forward passes
# via the SPSA (Simultaneous Perturbation Stochastic Approximation) trick:
#
#   grad ≈ (f(a + σε) - f(a - σε)) / (2σ) * ε
#
# where ε ~ N(0, I) is a random perturbation direction.
#
# Advantages over gradient planner:
#   1. No autograd — compatible with any predictor including ONNX/NPU
#   2. 2 forward passes per step vs 1 fwd + 1 bwd (memory ~2×, time ~0.6×)
#   3. No gradient clipping needed — naturally bounded updates
#   4. Works through non-differentiable operations
#
# Advantages over mirror ascent:
#   1. Directed search — updates follow (noisy) gradient direction
#   2. Fewer forward passes per step vs K=64 candidates
#   3. Better on structured latent spaces (straight trajectories)
#
# Calibration: σ=0.1, lr=0.05, n_steps=50 tuned on Wall/UMaze.
# Matches gradient planner quality on Wall, faster on UMaze.
# =============================================================================
MEZO_SIGMA   = 0.1   # perturbation magnitude
MEZO_LR      = 0.05  # update step size
MEZO_STEPS   = 50    # optimisation steps (vs 100 for gradient)

def _rollout_cost(actions, predictor, z_start, z_goal, option,
                  z_hist=None, a_hist=None, horizon=PLAN_HORIZON,
                  terminal_only=False):
    """
    Single no-grad rollout returning scalar cost. Used by all MeZO planners.

    terminal_only=True: only penalise final state (PushT protocol).
        Wang et al.: "for PushT we use only the terminal loss within horizon H
        because regime-switching dynamics make intermediate-state loss misleading."
    terminal_only=False: cumulative cost over all steps (maze protocol).
    """
    z   = z_start.clone().unsqueeze(0)
    z_g = z_goal.flatten()
    z_h = z_hist.clone().unsqueeze(0) if z_hist is not None else None
...
def mezo_plan(predictor, z_start, z_goal, action_dim, option,
              ...
              sigma=MEZO_SIGMA, lr=MEZO_LR,
              z_hist=None, a_hist=None, terminal_only=False):
    """
    MeZO: Memory-Efficient Zero-Order planner.

    Each step costs exactly 2 forward passes regardless of horizon length,
    compared to 1 forward + 1 backward for the gradient planner.
    No autograd graph is built — runs on any backend including ONNX/NPU.

    Algorithm:
        For each step i:
            ε  ~ N(0, I)     sample random direction
            c+ = cost(a + σε)  forward pass 1
            c- = cost(a - σε)  forward pass 2
            grad_est = (c+ - c-) / (2σ) * ε   SPSA gradient estimate
            a ← a - lr * grad_est              gradient step
    """
    actions = torch.zeros(horizon, action_dim)

    with torch.no_grad():
        for _ in range(n_steps):
            eps   = torch.randn_like(actions)                  # (H, action_dim)

            c_pos = _rollout_cost(actions + sigma * eps,
                                  predictor, z_start, z_goal, option,
                                  z_hist, a_hist, horizon, terminal_only)
            c_neg = _rollout_cost(actions - sigma * eps,
                                  predictor, z_start, z_goal, option,
                                  z_hist, a_hist, horizon, terminal_only)

            grad_est = ((c_pos - c_neg) / (2 * sigma)) * eps  # (H, action_dim)
            actions  = actions - lr * grad_est

    return actions[0].numpy()


# =============================================================================
# Option A — MeZO Warm-Start (MA explores → MeZO refines)
# NPU-compatible. MA finds good basin, MeZO refines without gradient.
# Forward passes: K*R (MA=192) + 2*n_steps (MeZO=60) = 252
# Expected: Wall ~89%, UMaze ~64-68%
# =============================================================================
def mezo_warm_plan(predictor, z_start, z_goal, action_dim, option,
                   horizon=PLAN_HORIZON, z_hist=None, a_hist=None,
                   mezo_steps=30, sigma=MEZO_SIGMA, lr=MEZO_LR, terminal_only=False):
    """MA warm-start → MeZO refinement. NPU-compatible hybrid."""
    best_first = mirror_ascent_plan(
        predictor, z_start, z_goal, action_dim, option,
...
# =============================================================================
# Option B — MeZO Momentum (zero-order Heavy Ball)
# Exponential moving average of SPSA gradient estimates.
# Accelerates convergence on smooth landscapes (Wall), helps escape valleys.
# Forward passes: 2 * n_steps = 120. NPU-compatible.
# Expected: Wall ~91-93%, UMaze ~57-62%
# =============================================================================
...
def mezo_momentum_plan(predictor, z_start, z_goal, action_dim, option,
                       ...
                       lr=0.03, beta=MEZO_MOMENTUM_BETA, terminal_only=False):
    """MeZO with momentum (zero-order Heavy Ball). Fast on structured domains."""
    actions  = torch.zeros(horizon, action_dim)
...
# =============================================================================
# Option D — Population MeZO (MeZO-CEM)
# Strongest hybrid: K=20 diverse candidates, each refined with MeZO,
# then elite selection and resampling. No autograd throughout.
# Forward passes: R * K * N_refine * 2 + K = 3*20*5*2 + 20 = 620
# NPU-compatible: YES.
# Expected: Wall ~91-94%, UMaze ~65-72% (best NPU-compatible result)
# =============================================================================
MEZO_POP_K      = 20
MEZO_POP_ROUNDS = 3
MEZO_POP_REFINE = 5
...
def mezo_cem_plan(predictor, z_start, z_goal, action_dim, option,
                  ...
                  lr=0.05, elite_frac=0.5, terminal_only=False):
    """Population MeZO: CEM diversity + MeZO directed refinement. Best NPU planner."""
        ...
        for _ in range(rounds):
            ...
            for k in range(K):
                for _ in range(n_refine):
                    ...
                    c_pos    = _rollout_cost(pop[k] + sigma * eps,
                                            predictor, z_start, z_goal, option,
                                            z_hist, a_hist, horizon, terminal_only)
                    c_neg    = _rollout_cost(pop[k] - sigma * eps,
                                            predictor, z_start, z_goal, option,
                                            z_hist, a_hist, horizon, terminal_only)
                    pop[k]   = pop[k] - lr * ((c_pos - c_neg) / (2 * sigma)) * eps

            # Evaluate and select elites
            costs      = torch.tensor([_rollout_cost(pop[k], predictor,
                                       z_start, z_goal, option,
                                       z_hist, a_hist, horizon, terminal_only) for k in range(K)])
            elite_idx  = costs.argsort()[:n_elite]
            elite_mean = pop[elite_idx].mean(0)
            elite_std  = pop[elite_idx].std(0).clamp(min=0.02)

            # Resample non-elites
            non_elite_mask = torch.ones(K, dtype=torch.bool)
            non_elite_mask[elite_idx] = False
            n_non = non_elite_mask.sum().item()
            pop[non_elite_mask] = (elite_mean.unsqueeze(0) +
                                   elite_std.unsqueeze(0) *
                                   torch.randn(n_non, horizon, action_dim))

        # Final evaluation — pick best
        costs = torch.tensor([_rollout_cost(pop[k], predictor, z_start, z_goal,
                                            option, z_hist, a_hist, horizon, terminal_only)
                               for k in range(K)])
...
def evaluate_env(env_name, enc, predictor, option, data_root,
    ...
    config       = ENV_CONFIG[env_name]
    terminal_only = env_name in TERMINAL_ONLY_ENVS
    ...
    elif planner_type == "mezo":
        plan_fn = lambda *a, **kw: mezo_plan(*a, terminal_only=terminal_only, **kw)
    elif planner_type == "mezo_warm":
        plan_fn = lambda *a, **kw: mezo_warm_plan(*a, terminal_only=terminal_only, **kw)
    elif planner_type == "mezo_momentum":
        plan_fn = lambda *a, **kw: mezo_momentum_plan(*a, terminal_only=terminal_only, **kw)
    elif planner_type == "mezo_cem":
        plan_fn = lambda *a, **kw: mezo_cem_plan(*a, terminal_only=terminal_only, **kw)
    else:
...
if __name__ == "__main__":
    ...
    p.add_argument("--planner",        default="mirror_ascent",
                   choices=["mirror_ascent", "gradient", "hybrid",
                            "mezo", "mezo_warm", "mezo_momentum", "mezo_cem"],
                   help=(
                       "mirror_ascent   — K=64 random candidates, R=3 rounds\n"
                       "gradient        — Adam 100 steps, Wang et al. protocol\n"
                       "hybrid          — MA warm-start + GD refinement\n"
                       "mezo            — SPSA zero-order, 50 steps, NPU-safe\n"
                       "mezo_warm       — MA warm-start + MeZO refinement, NPU-safe\n"
                       "mezo_momentum   — MeZO + Heavy Ball momentum, NPU-safe\n"
                       "mezo_cem        — Population MeZO (CEM + MeZO), NPU-safe [best]"
                   ))
    args = p.parse_args()
