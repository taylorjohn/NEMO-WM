...
import pynwb
import os
# Phase 12: Rust Math Engine with pure-Python fallback
try:
    import cortex_math
    _RUST_MATH = True
    ...
    class _CortexMathFallback:
        """Pure-Python z-score — identical semantics to cortex_math.calculate_z_score."""
        def calculate_z_score(self, history: list, current: float) -> float:
            if len(history) < 2:
                return 0.0
            arr = list(history) + [current]
            mu  = sum(arr) / len(arr)
            std = (sum((x - mu) ** 2 for x in arr) / len(arr)) ** 0.5
            return (current - mu) / std if std > 1e-9 else 0.0
    cortex_math = _CortexMathFallback()
...
class CortexGoldenMaster:
    def __init__(self):
        ...
        self._order_pending     = False  # entry lock — blocks new orders until sync confirms
        # Snapshot retry / stale-data state (Phase 2 timeout fix)
        self._snap_fail_count   = 0          # consecutive snapshot failures
        self._snap_backoff      = 3.0        # current backoff seconds (doubles on fail)
        self._snap_backoff_max  = 60.0       # cap at 60s
        self._snap_last_attempt = 0.0        # perf_counter of last attempt
        self._last_good_quote   = None       # most recent successful quote object
        self._last_good_price   = None       # float fallback price
        self._last_good_ts      = 0.0        # perf_counter when we got it
        ...
        # Phase 12 Warm-up
        if _RUST_MATH:
            print("⚡ Priming Rust Engine (compiled) and Phase 13 Toxicity Filters...")
            for _ in range(5): cortex_math.calculate_z_score([1.0, 2.0], 1.5)
        else:
            print("⚡ Rust Engine not compiled — using Python math fallback (Phase 12)")
            print("   To compile: cd CORTEX && maturin develop --release")
    ...
    def live_execute(self, z_score):
        ...
        # ── Phase 2 snapshot fetch with exponential backoff ──────────────────
        now = time.perf_counter()
        if now - self._snap_last_attempt < self._snap_backoff:
            # Still in backoff window — use stale data if available
            if self._last_good_quote is None:
                return   # no data at all yet
            quote = self._last_good_quote
            stale_age = now - self._last_good_ts
        else:
            stale_age = 0.0
            self._snap_last_attempt = now
            try:
                ...
                self._last_good_price = (float(quote.bp) + float(quote.ap)) / 2.0
                self._last_good_ts    = now
                self._snap_fail_count = 0
                self._snap_backoff    = 3.0
            except Exception as e:
                self._snap_fail_count += 1
                self._snap_backoff     = min(
                    self._snap_backoff * 2.0, self._snap_backoff_max
                )
                print(f"⚠️ Snapshot fail #{self._snap_fail_count} "
                      f"(backoff={self._snap_backoff:.0f}s): {e}")
                if self._last_good_quote is not None:
                    stale_age = now - self._last_good_ts
                    print(f"   Using stale quote (age={stale_age:.1f}s, "
                          f"price=${self._last_good_price:.2f})")
                    quote = self._last_good_quote
                else:
                    return  # no fallback data — abort tick

        try:
            # Phase 2 & 4: Latency Watchdog — apply to fresh quotes only
            if stale_age == 0.0:
                if self.last_rtt > 45.0:
                    return
                if (time.time() - quote.t.timestamp()) > 1.5:
                    return
            elif stale_age > 5.0:
                # Stale data older than 5s: allow only defensive exits, no entries
                if self.cached_qty == 0:
                    return   # no position to protect — skip
            ...
            if (z_score > Z_THRESHOLD or is_eod_panic or self.is_toxic_state) and self.cached_qty > 0:
                ...
                self.last_account_sync = 0.0   # force immediate re-sync on next tick
                print(f"🚨 EXIT TRIGGERED: {z_score:.2f}σ | Price: ${exit_p}")

            # --- ENTRY LOGIC ---
            elif z_score <= Z_THRESHOLD and self.cached_qty == 0 and not is_eod_panic and not self.is_toxic_state and not self._order_pending:
                if self.dynamic_z_threshold != -999.0 and z_score <= (2.92 - (self.dynamic_z_threshold - 2.92)):
                    ...
                    if qty > 0.0001:
                        ...
                        self.active_order_id, self.order_submit_time = order.id, time.perf_counter()
                        self.last_account_sync = 0.0   # force immediate re-sync on next tick
                        self._order_pending    = True  # lock until cached_qty confirmed
    ...
    def update_account_state(self):
        ...
        try:
            ...
            self._order_pending    = False  # position confirmed — allow new entries
        except: pass

if __name__ == "__main__":
    CortexGoldenMaster().run()
